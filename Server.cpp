...
void ProcessNewMessage(int nClientSocket) {
    ...
    if (nRet < 0) {
        ...
        cout << "Client " << nClientSocket << " disconnected." << endl;
        close(nClientSocket);
        // Remove client from pairs and notify paired client...
    } else {
        // Check if the message is empty
        if (strlen(buff) == 0) {
            cout << "Received an empty message from client: " << nClientSocket << endl;
            return; // Ignore empty messages
        }

        int pairedClient = clientPairs[nClientSocket];
        if (pairedClient != 0) {
            send(pairedClient, buff, nRet, 0); // Send actual message length
        }
        cout << "\n******************************************************************\n";
    }
}
    ...
    if (FD_ISSET(nSocket, &fr)) {
        ...
        if (nClientSocket >= 0) {
            if (availableClients.empty()) {
                ...
                send(nClientSocket, "Waiting for a partner... :)", 30, 0);
            } else {
